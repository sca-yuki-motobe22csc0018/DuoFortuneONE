...
public class DefenceWindow : MonoBehaviour
{
    ...
    public IEnumerator ShowDefenceChoice(PlayerManager player, CardGenerator.CardData cardData)
    {
        ...
        // ✅ DEFENCE かつ「コスト封印されていない」かつ「ライフDEFENCE封印されていない」時だけ使用可能
    ...
    private void OnUseClicked()
    {
        ...
        if (currentCardData != null)
        {
            ...
            {
                Debug.Log($"DefenceWindow: コスト {currentCardData.cost} は封印中のため使用不可");
                return;
            }
        }
        ...
        {
            Debug.Log("DefenceWindow: ライフDEFENCE封印中のため使用不可");
            return;
        }
